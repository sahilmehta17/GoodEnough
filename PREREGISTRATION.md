...
|---|---|---|---|
| 2026-08-07 | Added a post-hoc rule to the GSM8K secondary analysis (section 6): a logistic slope is reported only if at least 5 responses fall in the rarer of the two outcomes. Below that floor the slope and its interval are suppressed from the report text and retained in `reports/gsm8k.json` flagged `slope_interpretable: false`. | Not specified in advance. Introduced after data collection, once the hosted fit was observed to be near separable: hosted answered 148 of 150 items correctly, so its slope was set by 2 responses and its bootstrap interval ran to a boundary. The floor is applied identically to both models rather than to the hosted model alone, and it conditions on the response counts, not on which model produced them. | GSM8K hosted slope only, which is now reported as not estimable. The local slope is unaffected: local has 31 responses in its rarer outcome and clears the floor, and its value (-0.512, 90% CI [-0.797, -0.298]) is unchanged. No MMLU result, no margin, no interval, and no verdict is touched. |
| 2026-08-10 | Recorded the confidence-interval convention used across the generated reports. Quantities named in this document (sections 2, 6, 8) report a one-sided 95% bound at each end, the two ends together forming a two-sided 90% interval. Quantities this document does not name, namely the oracle gap in `reports/router.md` and the difficulty correlation in `reports/map.md`, report a two-sided 95% interval. Every generated report now states the level and the sidedness where each interval is printed, and `reports/map.md` carries the convention in full. | Both levels were already in the code and produced correct numbers, but were labelled inconsistently across files, because a one-sided 95% bound and one end of a two-sided 90% interval are the same number. The choice of two-sided 95% for the quantities this document does not name was made after data collection, so it is a post-hoc analysis decision and is recorded rather than left to be inferred from a tail parameter in the source. | None. No interval was recomputed and no verdict, margin, or reported value changed. Labels only. |
| 2026-08-10 | Aligned the GSM8K slope bootstrap (section 6) to the 10,000 resamples section 8 fixes for the paired bootstrap. It had been running at the 2,000-resample default of `analysis.bootstrap_slope`. Seed 42 and the one-sided 95% per-end level are unchanged, and the resample count and seed are now printed in `reports/gsm8k.md` alongside the interval. Resampling of already-collected responses only; no new data was collected. | The resample count was never a deliberate choice, only an unexamined function default, and it disagreed with the count this document fixes for every other bootstrap in the project. | GSM8K local slope interval only. It moves from [-0.797, -0.298] to **[-0.786, -0.288]**, which **supersedes the interval quoted incidentally in the 2026-08-07 row above**; that row is left unedited and its quoted interval should be read as the 2,000-resample value it was at the time. The point estimate is unchanged at -0.512, since the logistic fit is deterministic and only the interval is resampled. The hosted slope remains not estimable under the 2026-08-07 floor. No MMLU result, no margin, and no verdict is touched. |
